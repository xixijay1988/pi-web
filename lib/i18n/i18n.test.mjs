...
test("translates full-screen Alignment Workspace", () => {
  ...
  assert.equal(translate("zh-CN", "team.workspace.coach"), "需求助手");
  assert.equal(translate("zh-CN", "team.workspace.thinking"), "需求助手正在思考…");
  assert.equal(translate("zh-CN", "team.workspace.start"), "开始讨论");
  assert.equal(translate("zh-CN", "team.workspace.applyClose"), "应用到表单并关闭");
  assert.equal(translate("zh-CN", "team.workspace.previewPrimaryPath"), "主要使用路径");
});

test("uses the Requirements Assistant terminology in Team alignment", () => {
  assert.equal(translate("zh-CN", "team.coach.title"), "需求对齐 · 需求助手");
  assert.equal(translate("zh-CN", "team.coach.coach"), "需求助手");
  assert.equal(translate("zh-CN", "team.coach.replyPlaceholder"), "回复需求助手…");
});

test("translates remaining shell and sidebar controls", () => {
  assert.equal(translate("zh-CN", "app.getStarted"), "开始使用");
  assert.equal(translate("zh-CN", "app.fullHistory"), "完整历史");
  assert.equal(translate("zh-CN", "app.sessionInfo"), "会话信息");
  assert.equal(translate("zh-CN", "app.selectSession"), "请从侧边栏选择一个会话");
  assert.equal(translate("zh-CN", "session.rename"), "重命名");
  assert.equal(translate("zh-CN", "session.noSessions"), "未找到会话");
  assert.equal(translate("zh-CN", "session.newWorktree"), "新建工作树…");
});

test("translates Chat, files, configuration, and acceptance UI", () => {
  assert.equal(translate("zh-CN", "chat.input.send"), "发送");
  assert.equal(translate("zh-CN", "chat.window.loadEarlier", { count: 50 }), "向上滚动加载更早消息（已隐藏 50 条）");
  assert.equal(translate("zh-CN", "chat.message.copy"), "复制");
  assert.equal(translate("zh-CN", "files.uploadFiles"), "上传文件");
  assert.equal(translate("zh-CN", "files.viewer.preview"), "预览");
  assert.equal(translate("zh-CN", "models.title"), "模型");
  assert.equal(translate("zh-CN", "extensions.skills.title"), "技能");
  assert.equal(translate("zh-CN", "extensions.skills.checkUpdates"), "检查更新");
  assert.equal(translate("zh-CN", "extensions.plugins.title"), "插件");
  assert.equal(translate("zh-CN", "team.acceptance.accept"), "验收通过");
  assert.equal(translate("zh-CN", "markdown.copy"), "复制");
});

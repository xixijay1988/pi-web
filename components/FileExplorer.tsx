...
function uploadFiles(
  ...
  return new Promise((resolve, reject) => {
    ...
    xhr.onabort = () => reject(new Error(cancelledMessage));
    xhr.onload = () => {
...
export const FileExplorer = forwardRef<FileExplorerHandle, Props>(function FileExplorer({
  ...
  const performUpload = useCallback(async (
    ...
    try {
      ...
      if (status < 200 || status >= 300) {
        throw new Error(data.error ?? t("files.upload.failed", { status }));
      }
  ...
  const prepareUpload = useCallback(async (files: File[]) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error ?? t("files.upload.checkFailed", { status: res.status }));
      ...
      {showUploadFeedback && (
        ...
        {uploadBusy && (
          <div
            role="status"
            aria-live="polite"
            aria-label={uploadPhase === "checking"
              ? t("files.upload.checking")
              : t("files.upload.uploadingProgress", { progress: uploadProgress })}
          >
        ...
        {pendingConflict && (
          <div role="alert" style={{ padding: 7, border: "1px solid color-mix(in srgb, #f59e0b 55%, var(--border))", borderRadius: 4, background: "color-mix(in srgb, #f59e0b 9%, var(--bg-panel))" }}>
            <div style={{ fontSize: 11, color: "var(--text)", lineHeight: 1.35, overflowWrap: "anywhere" }}>
              {t(
                pendingConflict.conflicts.length === 1
                  ? "files.upload.conflictOne"
                  : "files.upload.conflictMany",
                {
                  count: pendingConflict.conflicts.length,
                  names: pendingConflict.conflicts.join(", "),
                },
              )}
            </div>
            {pendingConflict.nonReplaceable.length > 0 && (
              <div style={{ marginTop: 3, fontSize: 10, color: "#f59e0b", lineHeight: 1.35, overflowWrap: "anywhere" }}>
                {t("files.upload.cannotReplace", { names: pendingConflict.nonReplaceable.join(", ") })}
              </div>
            )}
        ...
        {uploadError && (
          <div role="alert" style={{ display: "flex", alignItems: "flex-start", gap: 6, fontSize: 11, lineHeight: 1.35, color: "#f87171" }}>
            ...
            <DismissButton onClick={() => setUploadError(null)} title={t("files.upload.dismissError")} />
          </div>
        )}
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 8, minHeight: 22, fontSize: 11 }}>
              <div style={{ minWidth: 0, flex: 1, display: "flex", alignItems: "center", gap: 8 }}>
                {uploadSummary.uploaded.length > 0 && (
                  <span title={t("files.upload.uploadedCount", { count: uploadSummary.uploaded.length })} aria-label={t("files.upload.uploadedCount", { count: uploadSummary.uploaded.length })} style={{ display: "flex", alignItems: "center", gap: 3, color: "#22c55e" }}>
                ...
                {uploadSummary.skipped.length > 0 && (
                  <span title={t("files.upload.skippedCount", { count: uploadSummary.skipped.length })} aria-label={t("files.upload.skippedCount", { count: uploadSummary.skipped.length })} style={{ display: "flex", alignItems: "center", gap: 3, color: "var(--text-dim)" }}>
                ...
                {uploadSummary.errors.length > 0 && (
                  <span title={t("files.upload.failedCount", { count: uploadSummary.errors.length })} aria-label={t("files.upload.failedCount", { count: uploadSummary.errors.length })} style={{ display: "flex", alignItems: "center", gap: 3, color: "#f87171" }}>
                    <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
              ...
              {uploadSummary.uploaded.length > 0 && onAtMentions && (
                <button
                  ...
                  title={uploadSummary.uploaded.length === 1 ? t("files.upload.addOneToChat") : t("files.upload.addManyToChat")}
                  aria-label={uploadSummary.uploaded.length === 1 ? t("files.upload.addOneToChat") : t("files.upload.addManyToChat")}
              ...
              <DismissButton onClick={() => setUploadSummary(null)} title={t("files.upload.dismissResults")} />
            </div>

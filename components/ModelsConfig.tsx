  ...
  return (
    <div style={{ position: "relative", width: "100%", ...style }}>
      ...
      <button
        ...
        aria-label={visible ? t("models.secret.hide") : t("models.secret.show")}
        title={visible ? t("models.secret.hide") : t("models.secret.show")}
        style={{
          position: "absolute",
          right: 5,
          top: "50%",
...
function Select({ value, onChange, options, required }: { value: string; onChange: (v: string) => void; options: readonly string[]; required?: boolean }) {
  ...
  return (
    <select value={value} onChange={(e) => onChange(e.target.value)}
      ...
      {!required && <option value="">{t("models.select.inheritNone")}</option>}
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <SectionTitle>{t("models.provider.section")}</SectionTitle>
      ...
      <Field label={t("models.provider.name")}>
        ...
        {editingName !== name && editingName.trim() && (
          <button onClick={() => onRename(editingName.trim())}
            ...
            {t("models.provider.rename")}
          </button>
        )}
      </Field>

      <Field label={t("models.provider.baseUrl")}>
        <TextInput value={provider.baseUrl ?? ""} onChange={(v) => set("baseUrl", v || undefined)}
          placeholder="https://api.example.com/v1" mono />
      </Field>

      <Field label={t("models.provider.apiKey")}>
        <SecretTextInput value={provider.apiKey ?? ""} onChange={(v) => set("apiKey", v || undefined)}
          placeholder={t("models.provider.apiKeyPlaceholder")} mono />
        <span style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>
          {t("models.provider.apiKeyHintBefore")} <code style={{ fontFamily: "var(--font-mono)" }}>!</code> {t("models.provider.apiKeyHintAfter")}
        </span>
      </Field>

      <Field label={t("models.provider.api")}>
          ...
          >
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 5, width: 68, flexShrink: 0 }}>
              ...
              <span style={{
                ...
                {t(`models.thinking.level.${level}` as never)}
              </span>
            </div>
...
function ModelDetail({
  ...
  const testSummary = (() => {
    ...
    if (testState.phase === "testing") return t("models.model.test.testingConnection");
    const meta = [
      testState.latencyMs !== undefined ? `${testState.latencyMs}ms` : null,
      testState.status !== undefined ? `HTTP ${testState.status}` : null,
    ].filter(Boolean);
    if (testState.phase === "success") {
      return [t("models.model.test.connected"), ...meta, testState.responseText || null].filter(Boolean).join(" · ");
    }
    return [t("models.model.test.failed"), ...meta, testState.message].filter(Boolean).join(" · ");
  })();
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <SectionTitle>{t("models.model.section")}</SectionTitle>
        <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button
            ...
            title={t("models.model.test.title")}
            style={{
              height: 24,
              padding: "0 8px",
          ...
          >
            ...
            {testState.phase === "testing"
              ? t("models.model.test.testing")
              : testState.phase === "success"
                ? t("models.model.test.ok")
                : t("models.model.test.action")}
          </button>
          <button onClick={onDelete}
            ...
            {t("models.model.remove")}
          </button>
        </div>
      </div>

      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Field label={t("models.model.id")}><TextInput value={model.id} onChange={(v) => set("id", v)} placeholder="model-id" mono /></Field>
        <Field label={t("models.model.name")}><TextInput value={model.name ?? ""} onChange={(v) => set("name", v || undefined)} placeholder={t("models.model.displayNamePlaceholder")} /></Field>
      </div>

      <Field label={t("models.model.apiOverride")}>
        <Select value={model.api ?? ""} onChange={(v) => set("api", v || undefined)} options={API_OPTIONS} />
      </Field>

      <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
        <Check label={t("models.model.reasoning")} checked={model.reasoning ?? false} onChange={(v) => set("reasoning", v || undefined)} />
        <Check label={t("models.model.imageInput")} checked={model.input?.includes("image") ?? false}
      ...
      {model.reasoning && (
        <>
          <Check
            label={t("models.model.deepSeekCompat")}
            checked={hasDeepseekCompat(model)}
            onChange={(v) => onChange(setDeepseekCompat(model, v))}
          />
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 8 }}>
              <SectionTitle>{t("models.model.thinkingLevelMap")}</SectionTitle>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
        <Field label={t("models.model.contextWindow")}>
          <NumInput value={model.contextWindow !== undefined ? String(model.contextWindow) : ""}
            onChange={(v) => set("contextWindow", v ? parseInt(v) : undefined)} placeholder="128000" />
        </Field>
        <Field label={t("models.model.maxOutputTokens")}>
      ...
      <div>
        <SectionTitle>{t("models.cost.title")}</SectionTitle>
        <div style={{ marginTop: 8, display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 8 }}>
          {(["input", "output", "cacheRead", "cacheWrite"] as const).map((k) => (
            <Field key={k} label={t(`models.cost.${k}` as never)}>
...
function OAuthDetail({ provider, onRefresh }: { provider: OAuthProvider; onRefresh: () => void }) {
  ...
  const handleLogin = useCallback(() => {
    ...
    es.onerror = () => {
      es.close();
      setLoginState((prev) => prev.phase === "success" ? prev : { phase: "error", message: t("models.oauth.connectionLost") });
    };
  ...
  const submitCode = useCallback(async (token: string, code: string) => {
    if (!code.trim()) return;
    setLoginState({ phase: "progress", message: t("models.oauth.verifying") });
    try {
      ...
      if (!res.ok) {
        ...
        setLoginState({ phase: "error", message: d.error ?? t("models.oauth.serverError", { status: res.status }) });
        return;
      }
      setInputValue("");
      // Success path: SSE stream will emit "success" and update state
    } catch (e) {
      setLoginState({ phase: "error", message: e instanceof Error ? e.message : t("models.oauth.networkError") });
    }
  }, [provider.id, t]);

  const submitSelection = useCallback(async (token: string, value: string) => {
    setLoginState({ phase: "progress", message: t("models.oauth.continuing") });
    try {
      const res = await fetch(`/api/auth/login/${encodeURIComponent(provider.id)}`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <SectionTitle>{t("models.oauth.section")}</SectionTitle>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: provider.loggedIn ? "#4ade80" : "var(--text-dim)" }}>
            {provider.loggedIn ? t("models.oauth.connected") : t("models.oauth.notConnected")}
          </span>
        </div>
      </div>
      ...
      <div style={{ minHeight: 48 }}>
        {loginState.phase === "idle" && (
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
            {provider.loggedIn
              ? t("models.oauth.alreadyConnected")
              : t("models.oauth.connectAccount", { provider: provider.name })}
          </p>
        )}
        {loginState.phase === "connecting" && (
          <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)" }}>{t("models.oauth.openingBrowser")}</p>
        )}
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
              {loginState.phase === "auth"
                ? t("models.oauth.completeSignIn")
                : loginState.message}
            </p>
            {loginState.phase === "auth" && (
              <p style={{ margin: 0, fontSize: 11, color: "var(--text-dim)", lineHeight: 1.5 }}>
                {t("models.oauth.browserDidNotOpen")}{" "}
                <a href={loginState.url} target="_blank" rel="noopener noreferrer" style={{ color: "var(--accent)", wordBreak: "break-all" }}>
                  {t("models.oauth.openLoginPage")}
                </a>
                .
              </p>
            )}
            <div style={{ display: "flex", gap: 6 }}>
              <input
                ...
                placeholder={loginState.phase === "auth" ? "http://localhost:1455/auth/callback?code=…" : (loginState.placeholder ?? t("models.oauth.enterValue"))}
        ...
        {loginState.phase === "device_code" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
              {t("models.oauth.openVerification")}
            </p>
            ...
            <p style={{ margin: 0, fontSize: 11, color: "var(--text-dim)", lineHeight: 1.5 }}>
              ...
              {loginState.expiresInSeconds
                ? ` ${t("models.oauth.expiresMinutes", { minutes: Math.ceil(loginState.expiresInSeconds / 60) })}`
                : ""}
            </p>
          </div>
        )}
        ...
        {loginState.phase === "success" && (
          <p style={{ margin: 0, fontSize: 12, color: "#4ade80" }}>{t("models.oauth.connectedSuccess")}</p>
        )}
        {loginState.phase === "error" && (
          <p style={{ margin: 0, fontSize: 12, color: "#f87171" }}>
            {t("models.oauth.errorContext", { error: loginState.message })}
          </p>
        )}
      </div>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {isWorking ? (
          ...
          <>
            ...
            >
              {provider.loggedIn ? t("models.oauth.relogin") : t("models.oauth.login")}
            </button>
            {provider.loggedIn && (
              <button
...
function ApiKeyDetail({ provider, onRefresh }: { provider: ApiKeyProvider; onRefresh: () => void }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <SectionTitle>{t("models.apiKey.section")}</SectionTitle>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          ...
          <span style={{ fontSize: 11, color: provider.configured ? "#4ade80" : "var(--text-dim)" }}>
            {provider.configured ? t("models.apiKey.configured") : t("models.apiKey.notConfigured")}
          </span>
        </div>
      </div>
      ...
      <p style={{ margin: 0, fontSize: 12, color: "var(--text-muted)", lineHeight: 1.5 }}>
        {provider.configured
          ? t("models.apiKey.storedHelp")
          : t("models.apiKey.enterHelp", { provider: provider.displayName, count: provider.modelCount })}
      </p>

      <Field label={t("models.apiKey.section")}>
        <div style={{ display: "flex", gap: 6 }}>
          <SecretTextInput
            value={apiKey}
            onChange={setApiKey}
            onKeyDown={(e) => { if (e.key === "Enter" && apiKey.trim()) handleSave(); }}
            placeholder={provider.configured ? t("models.apiKey.replacePlaceholder") : "sk-…"}
            style={{ flex: 1 }}
            autoComplete="off"
            spellCheck={false}
            mono
      ...
      {error && (
        <p style={{ margin: 0, fontSize: 12, color: "#f87171" }}>
          {t("models.apiKey.errorContext", { error })}
        </p>
      )}
      ...
      {provider.configured && (
        ...
        >
          {removing ? t("models.apiKey.remove") : t("models.apiKey.disconnect")}
        </button>
      )}
    </div>
  );
...
function AddProviderPicker({
  ...
  const customSearchText = [
    ...
    t("models.picker.custom"),
    t("models.picker.compatible"),
  ].join(" ").toLowerCase();
  const showCustom = !q || customSearchText.includes(q);
    ...
    >
      <div style={{ width: 820, maxWidth: "calc(100vw - 32px)", maxHeight: "min(72vh, calc(100vh - 32px))", background: "var(--bg)", border: "1px solid var(--border)", borderRadius: 10, display: "flex", flexDirection: "column", boxShadow: "0 8px 32px rgba(0,0,0,0.22)", overflow: "hidden" }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 14 }}>
          {totalCount === 0 ? (
            <div style={{ padding: "20px 0", fontSize: 12, color: "var(--text-dim)", textAlign: "center" }}>{t("models.picker.noMatches")}</div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(min(240px, 100%), 1fr))", gap: 8 }}>
              {showCustom && (
                <div style={{ gridColumn: "1 / -1", fontSize: 10, fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("models.picker.custom")}</div>
              )}
              {showCustom && (
                <button
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12, fontWeight: 600, color: "var(--text)", lineHeight: 1.3, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{t("models.picker.compatible")}</div>
                    <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>{t("models.picker.customEndpoint")}</div>
                  </div>
              ...
              {availableOAuth.length > 0 && (
                <div style={{ gridColumn: "1 / -1", paddingTop: showCustom ? 6 : 0, fontSize: 10, fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("models.picker.subscriptions")}</div>
              )}
              ...
              {availableApiKey.length > 0 && (
                <div style={{ gridColumn: "1 / -1", paddingTop: availableOAuth.length > 0 ? 6 : 0, fontSize: 10, fontWeight: 600, color: "var(--text-dim)", textTransform: "uppercase", letterSpacing: "0.07em" }}>{t("models.picker.apiKey")}</div>
              )}
              {availableApiKey.map((p) => (
                ...
                >
                  <div style={{ flex: 1, minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: 10, color: "var(--text-dim)", marginTop: 2 }}>{t("models.picker.modelCount", { count: p.modelCount })}</div>
                  </div>
                  <ProviderIcon id={p.id} size={28} />
                </button>
              ))}
              ...
              {loading ? (
                ...
                return (
                  <div key={pName} style={{ marginBottom: 2 }}>
                      ...
                      return (
                        ...
                        >
                          <span style={{ fontSize: 11, fontFamily: "var(--font-mono)", color: m.id ? "var(--text-muted)" : "var(--text-dim)", flex: 1, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                            {m.id || t("models.provider.newModel")}
                          </span>
                          {m.reasoning && (
                    ...
                    >
                      <span style={{ fontSize: 11 }}>{t("models.provider.addModel")}</span>
                    </div>
                  </div>
                );
              })}
          ...
          <div style={{ flex: 1, overflowY: "auto", padding: 20 }}>
            {loading ? null : detailContent ?? (
              <div style={{ height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "var(--text-dim)", fontSize: 13 }}>
                {t("models.provider.selectPrompt")}
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ display: "flex", alignItems: "center", justifyContent: "flex-end", gap: 10, padding: "10px 18px", borderTop: "1px solid var(--border)", flexShrink: 0 }}>
          {saveError && (
            <span style={{ fontSize: 12, color: "#f87171", flex: 1 }}>
              {t("models.saveError", { error: saveError })}
            </span>
          )}
